...
Beyond [`getWidth`](api/window.md#getWidth), [`getHeight`](api/window.md#getHeight) and [`getSize`](api/window.md#getSize), there are controls for the window itself. [`setTitle`](api/window.md#setTitle) sets the title, [`setSize`](api/window.md#setSize) resizes it and [`setResizable`](api/window.md#setResizable) decides whether the user can. [`setFullscreen`](api/window.md#setFullscreen) switches to fullscreen and back, and [`isFullscreen`](api/window.md#isFullscreen) reads the state. Fullscreen covers the whole display at its native resolution and `getWidth`/`getHeight` update to that size, so anything you size off them follows along; on macOS it is a borderless full-display fullscreen that fills the screen under the menu bar or notch rather than leaving a strip. [`minimize`](api/window.md#minimize), [`maximize`](api/window.md#maximize) and [`restore`](api/window.md#restore) do what they say. [`getDesktopDimensions`](api/window.md#getDesktopDimensions) gives the primary display's resolution, [`setIcon`](api/window.md#setIcon) takes an [`ImageData`](api/imagedata.md#ImageData) for the window icon, and [`showMessageBox`](api/window.md#showMessageBox) pops up a simple message and waits for it to be dismissed.
